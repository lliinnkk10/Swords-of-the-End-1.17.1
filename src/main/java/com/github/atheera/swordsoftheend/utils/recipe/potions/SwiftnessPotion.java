...
public class SwiftnessPotion implements IBrewingRecipe {

    private static final ItemStack INGREDIENT = new ItemStack(BlockInit.ITEM_BLOCK_MAGIC.get());
    private static final ItemStack OUTPUT = new ItemStack(ItemInit.ITEM_POTION_SWIFTNESS.get());

    public SwiftnessPotion() { }

    @Override
    public boolean isInput(@Nonnull ItemStack bottles) {
        return PotionUtils.getPotion(bottles) == Potions.STRONG_SWIFTNESS;
    }
