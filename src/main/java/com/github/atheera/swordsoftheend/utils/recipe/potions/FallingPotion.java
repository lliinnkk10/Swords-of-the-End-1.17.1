...
public class FallingPotion implements IBrewingRecipe {

    private static final ItemStack INGREDIENT = new ItemStack(BlockInit.ITEM_BLOCK_MAGIC.get());
    private static final ItemStack OUTPUT = new ItemStack(ItemInit.ITEM_POTION_FALLING.get());

    public FallingPotion() { }

    @Override
    public boolean isInput(@Nonnull ItemStack bottles) {
        return PotionUtils.getPotion(bottles) == Potions.LONG_SLOW_FALLING;
    }
